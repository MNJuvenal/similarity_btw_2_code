...
def load_pdgs_by_src(path):
    pdgs_by_src = {}
    for src_path in find_all_srcs(path):
        print(">>> Parsing " + src_path)

        remove("/tmp/gplag-db")
        remove("/tmp/gplag-pdg")

        parse_prog(src_path, "/tmp/gplag-db")
        pdgs_by_src[src_path] = load_pdg("/tmp/gplag-db", "/tmp/gplag-pdg")
